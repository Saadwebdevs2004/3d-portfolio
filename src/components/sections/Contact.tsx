    ...
    >
      ...
      <div className="max-w-6xl w-full mx-auto my-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center text-left">
        <div className="lg:col-span-7">
          <span className="text-xs font-mono tracking-widest text-[#FF6B4A] uppercase mb-4 block">
            04 // GET IN TOUCH
          </span>
          <h2 className="text-5xl md:text-7xl font-bold tracking-tight text-[#1A1A2E] uppercase font-heading mb-6">
            Let's build <br />
            <span className="text-gradient-accent">something epic.</span>
          </h2>
          <p className="text-[#4A4A6A] font-sans text-base md:text-lg max-w-lg leading-relaxed mb-8">
          ...
          <div className="flex flex-col gap-4 mb-8 font-mono">
            <div className="group inline-flex items-center gap-2">
              <a 
                ...
                className="text-xl md:text-3xl text-[#1A1A2E] hover:text-[#FF6B4A] transition-colors duration-300 border-b border-[#1A1A2E]/10 pb-2 relative overflow-hidden"
              >
                {emailAddress}
              </a>
              <svg className="w-6 h-6 text-[#4A4A6A] group-hover:text-[#FF6B4A] group-hover:translate-x-1 group-hover:-translate-y-1 transition-all duration-300 stroke-current fill-none" strokeWidth="2" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><line x1="7" y1="17" x2="17" y2="7"></line><polyline points="7 7 17 7 17 17"></polyline></svg>
            </div>
            
            <div className="text-sm mt-2 flex flex-col sm:flex-row sm:gap-8 gap-4">
              <div>
                <span className="text-[#4A4A6A] block text-[10px] uppercase tracking-widest mb-1">Phone</span>
                <a href={`tel:${phoneNumber}`} className="text-[#1A1A2E] hover:text-[#FF6B4A] transition-colors">{phoneNumber}</a>
              </div>
              <div>
                <span className="text-[#4A4A6A] block text-[10px] uppercase tracking-widest mb-1">Location</span>
                <span className="text-[#1A1A2E]">{locationText}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 flex flex-col md:flex-row lg:flex-col gap-6 items-start lg:items-end justify-center">
          <div className="flex gap-6">
            <MagneticButton>
              <a 
                href={`mailto:${emailAddress}`}
                className="w-16 h-16 rounded-full border border-[#FF6B4A]/25 bg-white flex items-center justify-center text-[#4A4A6A] hover:text-white hover:bg-[#FF6B4A] hover:border-[#FF6B4A] transition-all shadow-[0_4px_15px_rgba(255,107,74,0.1)] cursor-pointer"
                title="Email Me"
              >
      ...
      {/* Footer Block */}
      <div className="max-w-6xl w-full mx-auto mt-16 pt-8 border-t border-[#1A1A2E]/15 flex flex-col md:flex-row justify-between items-center gap-4 text-[#4A4A6A] font-mono text-[10px] md:text-xs text-center md:text-left">
        <div>
        ...
        <div className="flex gap-8">
          ...
          <button 
            ...
            className="hover:text-[#FF6B4A] flex items-center gap-1 cursor-pointer transition-colors"
          >
            BACK TO TOP ↑
          </button>
        </div>
